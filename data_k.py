...
class Datasets():
  ...
  def __init__(self, list_adjs,self_for_none, list_Xs, padding =True):
        """
  ...
  def process(self,index,self_for_none, padded_to=None,):
      ...
      if self.list_Xs == None:
          ...
          raise ("sorry, it should be implemented")
          # X = np.zeros((self.max_num_nodes, self.max_num_nodes))
          # X[:, :] = 0
          # X[:num_nodes, :num_nodes] = self.list_Xs[index][:num_nodes, :num_nodes]
...
if __name__ == '__main__':
    list_graph_loader( "DD", _max_list_size=500)
#     # Parameters
